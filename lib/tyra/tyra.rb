...
class Tyra
  ...
  def delegate(command)
    ...
    when "get_data" then Retriever.new(@base_db).get_data(command["dimension"], command["xaxis"], command["op"], command["xaxislabels"],
                                                          command["caxis"], command["caxislabels"], command["laxis"], command["laxislabels"])
